...
Deno.test({
  ...
  fn() {
    ...
    testList.forEach((c: number, i: number) =>
      assertEquals(c, testArr[i] ** 2)
    );
  },
});
...
Deno.test({
  name:
    "js_test/nested/nested/doublylinkedlist.js -> DoublyLinkedList.map (line 192)",
  ignore: false,
  fn() {
    const testArr = [1, 2, 3, 4, 5, 6, 78, 9, 0, 65];
...
//   { "failFast": true, "reportToConsole": false, "disableLog": true },
// );
